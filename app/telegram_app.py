...
def add(bot, message: Message):
    ...
    r = utils.write_to_sheet(url, sheet_name, data)
    try:
...
def remove(bot, message:Message):
    try:
        ...
        return
    url = "https://docs.google.com/spreadsheets/d/1FfZOET9rwsTFiSINmbacgC-XabNOt0XSS9v6kIyi8wA/edit#gid=1069132846"
    utils.delete_row(url, sheet_name, row_index)
    try:
...
def get_record(bot, message:Message):
    try:
        ...
        return
    url = "https://docs.google.com/spreadsheets/d/1FfZOET9rwsTFiSINmbacgC-XabNOt0XSS9v6kIyi8wA/edit#gid=1069132846"
    data,len_all = utils.get_last_record(url, sheet_name,10)
    custom_indices = range(len_all-8, len_all+2)
    # Chuyển đổi dữ liệu thành bảng sử dụng tabulate
    table = tabulate(utils.convert_dict_to_list(data), headers=["Row","Name", "Amount","Time"],tablefmt="simple", 
...
if __name__ == '__main__':
    ...
    data,len_all = utils.get_last_record(url, "11/2023", 10)
    custom_indices = range(len_all-8, len_all+2)
    data = utils.convert_dict_to_list(data)
